...
let topWatchlistedMoviesAmongFriends = (userId) => {
  // get user's friends.
  let friends = users.filter(object => object.userId === userId)[0].friends;
  let moviesWatchedByFriends = [];

  friends.forEach(friend => {
    let watchedMovies = movies.filter(movie => movie.watchlist.indexOf(friend) > -1);
    moviesWatchedByFriends.push(...watchedMovies);
  })

  console.log(friends);
  console.log(moviesWatchedByFriends);
}
...
// E.g. topWatchlistedMoviesAmongFriends(62289) ==> ["Schindler's List", "Pulp Fiction", "The Dark Knight", "The Shawshank Redemption"]
...
// E.g. topWatchlistedMoviesAmongFriends(15291) ==> ["The Dark Knight", "Schindler's List", "The Shawshank Redemption", "Pulp Fiction"]
...
// Pulp Fiction and The Godfather have both been watched once, 
// but Pulp fiction comes earlier alphabetically ("P.." vs "T.."), so it makes the top 4.